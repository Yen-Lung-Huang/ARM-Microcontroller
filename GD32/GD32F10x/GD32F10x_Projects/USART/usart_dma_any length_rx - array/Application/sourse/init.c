...
void usart_dma_config(uint8_t* dma_buffer, uint32_t transfer_amount)
{
    ...
    dma_init_struct.number = transfer_amount;							//傳輸的總數量
    ...
    dma_init_struct.priority = DMA_PRIORITY_HIGH;       //DMA優先級高
	  ...
	  dma_circulation_disable(DMA0, DMA_CH4);
		...
		dma_transfer_number_config(DMA0, DMA_CH4, transfer_amount); // Maybe it doesn't matter
...
bool irq(uint8_t* dma_buffer, uint8_t* rx_buffer, uint32_t transfer_amount)
{
		if(RESET != usart_interrupt_flag_get(USART0, USART_INT_FLAG_IDLE))
		{
	      ...
	      usart_data_receive(USART0);
				...
				usart_data_num = transfer_amount - dma_transfer_number_get(DMA0, DMA_CH4);
				...
				dma_transfer_number_config(DMA0, DMA_CH4, transfer_amount);
