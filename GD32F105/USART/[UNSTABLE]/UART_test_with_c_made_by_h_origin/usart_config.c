...
#define UART1_BUF_LEN 32				/* 單個緩存區位元組數 */
/* 定義發送緩存 */
uint8_t UART1_TX_BUF[UART1_BUF_LEN]; 		/* 發送緩衝區 */
uint8_t DMA_BUF_BUSY = 0 ;					/* 緩衝區是否已被佔用 */


/* 定義接收緩存 */

uint8_t UART1_RX_BUF[UART1_BUF_LEN*2]; /* 雙接收緩衝區 */
uint8_t UART1_RX_STAT = 0;					/* 接受狀態 0x01:已接收到資料  0x03:接收緩衝區半滿  0x07:接收緩衝區全滿 */
uint32_t UART1_RX_NUM = 0;					/* 接收到的資料個數 */
uint8_t rx1_data_buffer[32];// 待處理串列數據緩衝區
...
{	
  ...
  rcu_periph_clock_enable(RCU_DMA0);    //使能 DMA 時鐘
	...
	usart_word_length_set(USART1, USART_WL_8BIT); // (預設可省)
	usart_stop_bit_set(USART1, USART_STB_1BIT); // (預設可省)
	usart_parity_config(USART1, USART_PM_NONE); // (預設可省)
	usart_hardware_flow_rts_config(USART1, USART_RTS_DISABLE); // (預設可省)
	usart_hardware_flow_cts_config(USART1, USART_CTS_DISABLE); // (預設可省)
    ...
    //dma_init_struct.number = UART1_TX_LEN;						/* 傳輸資料個數 */ // DMA通道數據傳輸量
		dma_init_struct.number = sizeof(UART1_TX_BUF);						/* 傳輸資料個數 */ // DMA通道數據傳輸量 
    ...
    //dma_init_struct.number = UART1_RX_LEN*2;					/* 傳輸資料個數 */
		dma_init_struct.number = sizeof(UART1_RX_BUF)*2;					/* 傳輸資料個數 */
...
void DMA0_Channel6_IRQHandler(void)
{
	...
	void DMA0_Channel5_IRQHandler(void)
	{
		...
		if(dma_interrupt_flag_get(DMA0, DMA_CH5, DMA_INT_FLAG_HTF)){
			UART1_RX_NUM = strlen((char*)UART1_RX_BUF);
			UART1_RX_BUF[strlen((char*)UART1_RX_BUF)] = '\0';		/* 添加字串結束符 */
		...
		if(dma_interrupt_flag_get(DMA0, DMA_CH5, DMA_INT_FLAG_FTF)){
			UART1_RX_NUM = strlen((char*)UART1_RX_BUF)*2-1;
			UART1_RX_BUF[strlen((char*)UART1_RX_BUF)*2-1] = '\0';	/* 添加字串結束符 */
