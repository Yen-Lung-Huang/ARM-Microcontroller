...
void ms_pwm_init(enum Motor_Shield_Type type)
{
  if (type == MS_V1) { // If using Motor Shield V1
    ...
    // The timers are for NUCLEO-L053R8.
    motor_shield_l29xx.M1.EN.PWM = pwm_constructor((PWM_TypeDef){.timer=&htim2 , .channel=TIM_CHANNEL_2, .pwm_min=DC_MOTOR_MIN,\
    .pwm_max=DC_MOTOR_MAX, .physical_min=DC_MOTOR_MIN, .physical_max=DC_MOTOR_MAX, .offset=0,.pwm_value=0,\
    .reverse=false, .complementary=false, .latch=false});
    motor_shield_l29xx.M2.EN.PWM = pwm_constructor((PWM_TypeDef){.timer=&htim22, .channel=TIM_CHANNEL_2, .pwm_min=DC_MOTOR_MIN,\
    .pwm_max=DC_MOTOR_MAX, .physical_min=DC_MOTOR_MIN, .physical_max=DC_MOTOR_MAX, .offset=0,.pwm_value=0,\
    .reverse=false, .complementary=false, .latch=false});
    motor_shield_l29xx.M3.EN.PWM = pwm_constructor((PWM_TypeDef){.timer=&htim22, .channel=TIM_CHANNEL_1, .pwm_min=DC_MOTOR_MIN,\
    .pwm_max=DC_MOTOR_MAX, .physical_min=DC_MOTOR_MIN, .physical_max=DC_MOTOR_MAX, .offset=0,.pwm_value=0,\
    .reverse=false, .complementary=false, .latch=false});
    motor_shield_l29xx.M4.EN.PWM = pwm_constructor((PWM_TypeDef){.timer=&htim2 , .channel=TIM_CHANNEL_1, .pwm_min=DC_MOTOR_MIN,\
    .pwm_max=DC_MOTOR_MAX, .physical_min=DC_MOTOR_MIN, .physical_max=DC_MOTOR_MAX, .offset=0,.pwm_value=0,\
    .reverse=false, .complementary=false, .latch=false});
