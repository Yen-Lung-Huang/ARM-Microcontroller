...
{
    // 檢查按鈕 B1 和 B2 的狀態，當其中一個被按下時停止 M1 馬達
    if (Button_IsPressed(&button[B1]) || Button_IsPressed(&button[B2])) {
        ms_motor_control(&motor_shield_v1, MS_V1, M1, 0); // 停止 M1 馬達
    }

    // 檢查按鈕 B3 和 B4 的狀態，當其中一個被按下時停止 M2 馬達
    if (Button_IsPressed(&button[B3]) || Button_IsPressed(&button[B4])) {
        ms_motor_control(&motor_shield_v1, MS_V1, M2, 0); // 停止 M2 馬達
    }

    // 檢查按鈕 B4 的狀態，當 B4 未被按下時停止 M3 馬達
    if (!Button_IsPressed(&button[B4])) {
        ms_motor_control(&motor_shield_v1, MS_V1, M3, 0); // 停止 M3 馬達
    }

    // 檢查按鈕 B3 的狀態，當 B3 未被按下時停止 M4 馬達
    if (!Button_IsPressed(&button[B3])) {
        ms_motor_control(&motor_shield_v1, MS_V1, M4, 0); // 停止 M4 馬達
    }
}
