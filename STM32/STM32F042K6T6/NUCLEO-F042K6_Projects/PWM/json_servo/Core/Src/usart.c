...
/* USER CODE BEGIN 0 */
uint8_t RxBuf[RxBuf_SIZE]; 			// Receive buffer
uint8_t MainBuf[MainBuf_SIZE];	// RX receive. 
uint8_t aRxBuf; 								// Receive buffer(Receive 1 character).
uint8_t debug_rx_cnt = 0;
//bool debug_rx_flag = true;
/* USER CODE END 0 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart == &huart2) {
		...
		HAL_UART_Receive_IT(huart, &aRxBuf, 1);
	}
}
/* USER CODE END 1 */
