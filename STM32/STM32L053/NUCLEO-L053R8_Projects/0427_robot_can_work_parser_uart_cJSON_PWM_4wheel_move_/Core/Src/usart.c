...
uint8_t move_pwm_value = 0;  //for uart comand change speed ex: {"move":255}
    ...
    if(root == NULL){
		...
		for (cJSON *token = root->child;token != NULL;token = token->next){
			...
			if(!strcmp(token->string,"move")){
				if(cJSON_IsNumber(cJSON_GetObjectItem(root,"move")) == true){ // move:200
							move_pwm_value = token->valuedouble; // Desired variable 
					 		TIM2->CCR1 = move_pwm_value;				//setbuf speed pwm value (0~255)
							TIM2->CCR2 = move_pwm_value;
							printf("%s:%d\r\n",token->string, move_pwm_value); // Do some stuff. // Print for debug.
            }
				else if(cJSON_IsString(cJSON_GetObjectItem(root,"move")) == true){			
					if(!strcmp(token->valuestring,"turn_left")){
					...
					else if(!strcmp(token->valuestring,"turn_right")){
						printf("%s:turn_right\r\n",token->string); // Do some stuff. // Print for debug.
						TIM2->CCR1 = move_pwm_value;				//turn speed pwm value !!!floor(*0.8)!!!
						TIM2->CCR2 = move_pwm_value;
						////M1
					...
					else if(!strcmp(token->valuestring,"backward")){
						printf("%s:backward\r\n",token->string); // Do some stuff. // Print for debug.
						//M1
						HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10, GPIO_PIN_RESET);
						HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);
					...
					else if(!strcmp(token->valuestring,"forward")){
						printf("%s:forward\r\n",token->string); // Do some stuff. // Print for debug.
						//M1
						HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10, GPIO_PIN_SET);
						HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET);
					...
					else if(!strcmp(token->valuestring,"stop")){
						printf("%s:stop\r\n",token->string); // Do some stuff. // Print for debug.
						//M1
						HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10, GPIO_PIN_SET);
						HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);
			...
			// root: rc_servo
			//{"rc_servo":{"1":75,"2":75,"3":75,"4":75,"5":200}}
