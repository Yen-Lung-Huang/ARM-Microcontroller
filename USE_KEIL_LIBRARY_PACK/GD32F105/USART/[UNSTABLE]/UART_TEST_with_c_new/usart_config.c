...
uint8_t USART1_TX_BUF[256];   // DMA發送緩衝區
uint8_t USART1_RX_BUF[256];   // DMA接收緩衝區
uint8_t rx1_data_buffer[256]; // 待處理串列數據緩衝區
...
void USART1_init(uint32_t baudrate)
{
  ...
  nvic_irq_enable(USART1_IRQn, 0, 0);	                                     // USART中斷優先級設置搶佔優先級0、子優先級0並啟用
	usart_interrupt_enable(USART1, USART_INT_TC|USART_INT_IDLE);             // 啟用發送完成中斷、空閒中斷
...
void USART1_vsprintf(const char* format,...)  
{
	...
	vsprintf((char*)USART1_TX_BUF,format,argptr);
	va_end(argptr);
	USART1_TX_BUF[sizeof(USART1_TX_BUF)-1] = '\0';
	
	/* 設置DMA傳輸 */
	dma_channel_disable(DMA0, DMA_CH6); // 關閉DMA傳輸纔可以進行設置
	dma_memory_address_config(DMA0,DMA_CH6,(uint32_t)(USART1_TX_BUF));
	dma_transfer_number_config(DMA0,DMA_CH6,strlen((const char*)(USART1_TX_BUF))); // strlen(...)：計算此次發送數據的長度
...
void DMA0_Channel5_IRQHandler(void)
	{
		/* 清除DMA0 通道5 中斷標誌位元 */
		uint32_t USART1_RX_LEN = sizeof(USART1_RX_BUF);
		dma_interrupt_flag_clear(DMA0, DMA_CH5, DMA_INT_FLAG_G);
		if(dma_interrupt_flag_get(DMA0, DMA_CH5, DMA_INT_FLAG_HTF)){
			USART1_RX_NUM = USART1_RX_LEN;
			USART1_RX_BUF[USART1_RX_LEN] = '\0';		/* 添加字串結束符 */
		...
		if(dma_interrupt_flag_get(DMA0, DMA_CH5, DMA_INT_FLAG_FTF)){
			USART1_RX_NUM = USART1_RX_LEN*2;
			USART1_RX_BUF[USART1_RX_LEN*2-1] = '\0';	/* 添加字串結束符 */
...
void USART1_IRQHandler(void)	//串列1中斷服務程序
{
	if(RESET != usart_interrupt_flag_get(USART1, USART_INT_FLAG_IDLE)) // 空閒中斷
	{
		...
		memcpy(rx1_data_buffer,USART1_RX_BUF,USART1_RX_NUM); // 轉存數據到待處理數據緩衝區
		rx1_data_buffer[USART1_RX_NUM] = '\0';	// 添加字符串結束符
